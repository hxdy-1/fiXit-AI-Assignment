...
const Sidebar = () => {
	...
	return (
		<motion.aside
			...
			initial={{ width: "15%" }}
			animate={{ width: isCollapsed ? "5%" : "15%" }}
			transition={{ duration: 0.3 }}
		>
			<nav
				className={`h-full w-full flex flex-col border-r gap-10 text-sm text-white py-8 ${
					isCollapsed ? "items-center px-3" : "px-4"
				}`}
			>
				<button className="mb-2 cursor-pointer" onClick={handleToggle}>
					<svg
				...
				<div className="flex flex-col gap-4 w-full">
					<p
						className={`text-blue-400 font-semibold capitalize ${
							isCollapsed ? "text-center" : ""
						}`}
					>
						{isCollapsed ? "CA" : "Call Audit"}
					</p>
					<div
						className={`flex ${
							isCollapsed ? "justify-center" : "gap-4"
						} items-center py-2 rounded transition-colors duration-300 hover:text-neutral-400 cursor-pointer`}
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							height="20"
					...
					>
						...
						{!isCollapsed && <span>Data score</span>}
					</div>
					<button
						className={`flex ${
							isCollapsed ? "justify-center" : "gap-4"
						} items-center border rounded-md py-4 ${
					...
					>
						...
						{!isCollapsed && <span>Upload call records</span>}
					</button>
				</div>
				<div className="flex flex-col gap-4 w-full">
					<p
					...
					<button
						className={`flex ${
							isCollapsed ? "justify-center" : "gap-4"
						} items-center justify-between border rounded-md py-4 px-4`}
					>
						<span>{isCollapsed ? "UB" : "Union_Budgets"}</span>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							height="14"
							width="14"
					...
					<button
						className={`flex ${
							isCollapsed ? "justify-center" : "gap-4"
						} items-center border rounded-md py-4 px-4`}
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							height="20"
